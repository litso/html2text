...
class HTML2Text(HTMLParser.HTMLParser):
    ...
    def handle_tag(self, tag, attrs, start):
        ...
        if tag in ['em', 'i', 'u']:
            if (not self.ignore_text_formatting):
                self.o("_")
        if tag in ['strong', 'b']:
            if (not self.ignore_text_formatting):
                self.o("**")
...
if __name__ == "__main__":
    ...
    p.add_option("--ignore-text-formatting", dest="ignore_text_formatting", action="store_true",
        default=IGNORE_TEXT_FORMATTING, help="don't include any text formatting")
    ...
    h.ignore_text_formatting = options.ignore_text_formatting
